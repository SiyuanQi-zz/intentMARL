    ...
    if pathCalculated==1 then
        if currentIndexOnPath < #targetPath-2 then
            ...
            while true do
                ...
                if computeDistance(currentTargetPos2d, nextPos) > 0.1 or currentIndexOnPath >= #targetPath-2 then
                    break
                end
...
function createPlanningSpace( )
    ...
    stateSpaceHandle = {simExtOMPL_createStateSpace('droneStateSpace', sim_ompl_statespacetype_position2d, heli, {0, 0}, {228, 382}, 1)}
    -- stateSpaceHandle = {simExtOMPL_createStateSpace('droneStateSpace', sim_ompl_statespacetype_position2d, targetObj, {-25, -25}, {25, 25}, 1)} -- For testing
    ...
    simExtOMPL_setCollisionPairs(taskHandle, {targetObj, droneObstacles})
