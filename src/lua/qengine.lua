...
if (sim_call_type==sim_childscriptcall_initialization) then
    ...
    droneHandle = simGetObjectHandle('Quadricopter')
    local droneScriptHandle = simGetScriptAssociatedWithObject(droneHandle)
    local targetHandle = simGetObjectHandle('Bill')
    local pos = simGetObjectPosition(targetHandle, -1)
    local ori = simGetObjectOrientation(targetHandle, -1)
    -- simCallScriptFunction('setTarget', droneScriptHandle, pos, ori)
    simCallScriptFunction('setTrackingTarget', droneScriptHandle, targetHandle)
